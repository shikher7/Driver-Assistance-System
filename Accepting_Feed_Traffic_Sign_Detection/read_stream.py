...
from driving_functions import sign_threshold, slide_window, predict_sign, barrier_motion
...
def run(model, conncetion, conncetion_2):
    ...
    try:
        while True:
            ...
            img = rotated180
            #Image.open(img).show()
            #cv2.imwrite('C:/Users/ayush/Desktop/Final Year Project/Automated-Driving-System-master/Accepting_Feed_Traffic_Sign_Detection/Images/'+str(time.time())+'.jpg', img)  # Save image
#            if state == 'Barrier':
...
#                flag = 1  # Printing flag
            ...
            if state == 'barrier_motion_detection':  # Barrier motion detection
                if first_frame_barrier is None:
                    sta = "Start"
                    first_frame_barrier = cv2.cvtColor(img, cv2.COLOR_BGRGRAY)
                    connection_2.send(sta.encode())
                else:
                    slope = barrier_motion(img, first_frame_barrier)
                    if slope > 1:
                        s = "Keep Moving"
                        connection_2.send(s.encode())
                        print('Keep driving!')
                        state = 'Normal'
                        flag = 1  # Printing flag
                    else:
                        stp = "Sto it mFUCKAp"
                        connection_2.send(stp.encode())
                ...
                for window in windows:
                    ...
                    if np.sum(mask[window[0][1]:window[1][1],window[0][0]:window[1][0]]) <= 100000:
                        ...
                        if pred_num == 3 and prob > 0.99:  # If prediction equals stop sign (==3) with high confidence (> 0.99)
                            cv2.imwrite('C:/Users/ayush/Desktop/Final Year Project/Automated-Driving-System-master/Accepting_Feed_Traffic_Sign_Detection/Images/'+str(time.time())+'.jpg', img) # Save cropped image of (predicted) stop sign
                            print(pred_num, "----", prob, '----', time.time())
                            stop_detect += 1  # To avoid false positives, initiate a detection count
                            if stop_detect == 1:
                                stop_first = time.time()
                                kp = "Chances of Stopping"# Time value at first stop sign detection
                                connection_2.send(kp.encode())  # Send no stop sign detected signal to Pi
                            elif stop_detect == 3:  # If 3 consecutive images were stop sign
                                if time.time() - stop_first < 2:
                                    ssd = "Stop Sign Detected"# if time between detections below 2 sec.
                                    connection_2.send(ssd.encode())  # Send stop sign detected signal to Pi
                                    print(pred_num, "----", prob, '----', time.time())
                                    state = 'Stop'  # Change state to barrier
                                    flag = 1  # Printing flag
                                    img_tiny = img_cropped_copy
                                    cv2.rectangle(img, window[0], window[1], (0,255,0), 4)


                            
                                else:
                                    stop_detect = 0
                                    k = "Keep Moving"# If no stop sign detected, set count=0
                                    connection_2.send(k.encode())  # Send no stop sign detected signal to Pi
                        else:
                            ko = "Keep Moving"
                            connection_2.send(ko.encode())  # Send no stop sign detected signal to Pi
#                    else:
#                        ki = "KM"
#                        connection_2.send(ki.encode())   # Send no stop sign detected signal to Pi
                if len(img_tiny) != 0:
                    y_offset = 60; x_offset = 0
                    img[y_offset:y_offset+img_tiny.shape[0], x_offset:x_offset+img_tiny.shape[1]] = img_tiny 
                cv2.imshow('image', img)
                cv2.waitKey(500)
